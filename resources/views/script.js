...
new Vue({
    ...
    methods: {
        ...
        saveEditingNote() {
            if (!this.editingNote.note.trim()) return alert("Please write a note!");
        ...
        deleteNote(note) {
            if (!confirm(`Really delete ${note.note}?`)) return;
            localStorage.removeItem(note.id);
            this.getNotes();
        },
        saveNote() {
            if (!this.newNote.note.trim()) return alert("Please write a note!");
            let id = this.uuid();
            this.newNote.id = id;
